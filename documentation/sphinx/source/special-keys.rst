...
#. ``\xff\xff/management/maintenance/<zone_id> := <seconds>`` Read/write. Set/clear a key in this range will change the corresponding system key ``\xff\x02/healthyZone``. The value is a literal text of a non-negative ``double`` which represents the remaining time for the zone to be in maintenance. Committing with an invalid value will throw ``special_keys_api_failure``. Only one zone or data hall is allowed to be in maintenance at the same time. Setting a new key in the range will override the old one and the transaction will throw ``special_keys_api_failure`` error if more than one target is given. For more details, see help text of ``fdbcli`` command ``maintenance``.
   ...
   To mark a data hall for maintenance, use the special key ``\xff\xff/management/maintenance/data_hall:<data_hall_id>`` with the desired duration in seconds. When a data hall is under maintenance, all storage servers in that data hall will be treated similarly to a zone under maintenance - data will not be moved away from failed processes in that data hall.
